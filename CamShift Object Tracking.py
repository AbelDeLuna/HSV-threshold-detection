...
class ObjectTracking(object):
    def __init__(self, scalingFactor):
        ...
        #init rectangular region selection
        self.selection = None
    ...
    def mouse_tracking(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            #set startling location on mouse click location
            self.dragStart = (x,y)
            self.trackingState = 0
    ...
    def start_tracking(self, lowerLimit, upperLimit):
        while(True):
            ...
            if self.trackingState == 1:
                ...
                #Apply CAMshift to histogram back projection
                trackBox, self.trackWindow = cv2.CamShift(hsvBackProjection, self.trackWindow, termCriteria)
                cv2.ellipse(vis, trackBox, (0,255,0),2)

            #subtractorMask = self.subtractor.apply(self.frame, learningRate = (.000000001))
            #subtractorMask = cv2.cvtColor(subtractorMask, cv2.COLOR_GRAY2BGR)
            cv2.imshow('tracking', vis)
            #cv2.imshow('subtractor', subtractorMask & vis)
